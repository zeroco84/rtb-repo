...
// S3 client — lazily initialised to avoid build-time module resolution issues
let _s3Client = null;
let _S3Loaded = false;
...
function postProcessResult(parsed, disputeInfo) {
    ...
    // Cross-check: if award_items exist, verify the total is in the right ballpark
    if (parsed.award_items && Array.isArray(parsed.award_items) && parsed.award_items.length > 0) {
        ...
        if (claimed > 0 && itemsTotal > 0) {
            const pctDiff = Math.abs(itemsTotal - claimed) / Math.max(itemsTotal, claimed);
            if (pctDiff > 0.25) {
                // More than 25% discrepancy — AI is confused, null it out
                console.warn(`[AI] Large amount mismatch for ${disputeInfo.dr_no}: total=${claimed}, items sum=${itemsTotal} (${(pctDiff * 100).toFixed(0)}% off). Setting to null.`);
                parsed.compensation_amount = null;
                parsed._uncertain = true;
            } else if (pctDiff > 0.01) {
                // Small discrepancy — trust the stated total, just log
                console.log(`[AI] Minor mismatch for ${disputeInfo.dr_no}: total=${claimed}, items=${itemsTotal} (${(pctDiff * 100).toFixed(0)}% off). Keeping stated total.`);
            }
        }
    }
...
export async function processDispute(dispute) {
    ...
    return {
        ...
        ai_compensation_amount: result._uncertain ? null : amount,
...
export async function processEnforcementOrder(order) {
    ...
    if (result.award_items && Array.isArray(result.award_items) && result.award_items.length > 0) {
        ...
        if (claimed > 0 && itemsTotal > 0) {
            const pctDiff = Math.abs(itemsTotal - claimed) / Math.max(itemsTotal, claimed);
            if (pctDiff > 0.25) {
                console.warn(`[AI] Large amount mismatch for enforcement ${order.court_ref_no}: total=${claimed}, items=${itemsTotal} (${(pctDiff * 100).toFixed(0)}% off). Setting to null.`);
                result.compensation_amount = null;
            } else if (pctDiff > 0.01) {
                console.log(`[AI] Minor mismatch for enforcement ${order.court_ref_no}: total=${claimed}, items=${itemsTotal}. Keeping stated total.`);
            }
        }
    }
