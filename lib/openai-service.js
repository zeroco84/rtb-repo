...
- "compensation_amount": The total monetary amount ordered by the court in euros (whole number, 0 if none or if the order is non-monetary). Copy amounts EXACTLY as written. If unsure, set to 0.
...
export async function processEnforcementOrder(order) {
    ...
    let pdfBase64;
    try {
        pdfBase64 = await downloadPdfAsBase64(order.pdf_url);
    } catch (err) {
    ...
    const amount = result.compensation_amount !== null ? Math.round(parseFloat(result.compensation_amount) || 0) : null;
