...
// ============================================
// NONCE FETCHER
// Nonces are per-session — must fetch fresh before each scrape batch.
// The nonce is embedded in a wp_localize_script object on the page.
// ============================================
async function fetchNonce() {
  ...
  try {
    ...
    // Pattern: "nonce":"XXXXXXXXXX"
    const match = html.match(/"nonce"\s*:\s*"([a-f0-9]+)"/);
    if (!match) throw new Error('Nonce not found in page HTML');
    console.log(`[RentRegister] Nonce fetched: ${match[1]}`);
    return match[1];
  } catch (err) {
    clearTimeout(timeoutId);
    throw new Error(`Failed to fetch nonce: ${err.message}`);
  }
}

// ============================================
// HEALTH CHECK
// Confirm backend is up before starting a batch
// ============================================
export async function checkHealth(nonce) {
...
// ============================================
// SINGLE SEARCH
// Calls comparables_calc_proxy for one combination
// ============================================
async function searchComparables(nonce, { osiLeaId, dwellingTypeCode, bedrooms, berRating = null, floorSpace = null }) {
  const controller = new AbortController();
  const timeoutId = setTimeout(() => controller.abort(), REQUEST_TIMEOUT);
  ...
  try {
    const payload = {
      ...
      BER: berRating,         // null = no BER filter (returns all BER ratings)
      FloorSpace: floorSpace, // null = no floor space filter
    };
...
export async function* scrapeRentRegisterDublin({ leaRefs, onProgress, maxBedrooms = 5 } = {}) {
  ...
  // Step 2: Health check
  const healthy = await checkHealth(nonce);
  ...
  for (const lea of leaRefs) {
    for (const dwellingType of DWELLING_TYPES) {
      for (let bedrooms = 1; bedrooms <= maxBedrooms; bedrooms++) {
        ...
        const results = await searchComparables(nonce, {
